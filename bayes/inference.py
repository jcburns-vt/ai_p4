...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        while len(eliminationOrder) != 0:
            eliminationVar = eliminationOrder.pop(0)
            print(eliminationVar)
            factors, joinedFactor = joinFactorsByVariable(factors, eliminationVar)
            if len(joinedFactor.unconditionedVariables()) > 1:
                joinedAndEliminatedFactor = eliminate(joinedFactor, eliminationVar)
                factors.append(joinedAndEliminatedFactor)

        remainingFactorsJoined = joinFactors(factors)
        normalizedResult = normalize(remainingFactorsJoined)
...
def sampleFromFactorRandomSource(randomSource=None):
    ...
    def sampleFromFactor(factor, conditionedAssignments=None):
        ...
        if conditionedAssignments is None and len(factor.conditionedVariables()) > 0:
            ...
            conditionedVariables = set([var for var in list(conditionedAssignments.keys())])
            ...
            for (var, assignment) in list(conditionedAssignments.items()):
