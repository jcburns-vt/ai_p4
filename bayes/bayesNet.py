...
class BayesNet(object):
    ...
    def linearizeVariables(self):
        ...
        Useful for sampling.
        """
        inEdgesIncremental = dict([(var, edgeSet.copy()) for (var, edgeSet) in list(self.__inEdges.items())])
    ...
    def getReducedVariableDomains(self, evidenceDict):
        ...
        for (evidenceVariable, value) in list(evidenceDict.items()):
    ...
    def getCPTWithEvidence(self, variable, evidenceDict=None):
        ...
        """
        if evidenceDict is None or len(list(evidenceDict.items())) == 0:
...
class Factor(object):
    ...
    def __getAssignmentsInOrder(self, assignmentDict):
        ...
        reducedAssignmentDict = dict([(var, val) for (var, val) \
                                      in list(assignmentDict.items()) if var in self.__variablesSet])
        variablesAndAssignments = list(reducedAssignmentDict.items())
        variablesAndAssignments = sorted(variablesAndAssignments, \
                                         key=lambda var_val : self.__variableOrders[var_val[0]])
    ...
    def getAllPossibleAssignmentDicts(self):
        ...
        return [dict(list(zip(reversed(self.__variables), product))) for product in cartesianProductOfAssignments]
    ...
    def easierToParseString(self, prefix=None, printVariableDomainsDict=True):
        ...
        if printVariableDomainsDict:
            ...
            for (key, domain) in list(self.__variableDomainsDict.items()):
    ...
    def specializeVariableDomains(self, newVariableDomainsDict):
        ...
        for (variable, domain) in list(newVariableDomainsDict.items()):
...
def constructEmptyBayesNetFromString(bayesNetString):
    ...
    edgeList = [(u, v) for u, _, v in map(tuple, list(map(str.split, edgeStrings)))]
...
def fillWithOneConditionedAssignmentRandomly(factor, unconditionedVariables, conditionedVariables, product, variableDomainsDict):
    ...
    for (randomFill, variableValue) in zip(randomFills, cartesianProductOfUnConditionalAssignments):
        assignmentDict = dict(list(zip(list(unconditionedVariables) + list(conditionedVariables), list(variableValue) + list(product))))
